<template>
  <header
    class="p-4 bg-white border-b border-border flex items-center justify-between"
  >
    <router-link to="/">
    ...
    <div class="flex gap-4" v-if="!isAuth">
      ...
      <router-link class="btn-auth bg-black2 text-white" to="/auth/signup">
        Signup
      </router-link>
    </div>
    <div v-else class="flex gap-4">
      <base-button link to="/admin/links">
        admin
        <appIcon name="admin" />
      </base-button>
      <base-button mode="err" @click="logout">
        logout
        <appIcon name="logout" />
      </base-button>
    </div>
